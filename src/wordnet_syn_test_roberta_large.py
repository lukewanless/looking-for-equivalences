...
if __name__ == '__main__':
    ...
    search_path = "hyperparams/roberta_large_{}/search_{}.csv".format(folder,
        search_random_state)
    assert os.path.exists(search_path)

    transformation_name = "wordnet syn tranformation p and h"
    output_raw_result = "raw_results/{}/roberta_large/syn_p_h/rho_{:.2f}_results".format(folder,
        rho)
    output_raw_result = output_raw_result.replace(".", "p") + ".csv"
    output_result = "results/{}/roberta_large/syn_p_h/rho_{:.2f}_results".format(folder,
        rho)
    output_result = output_result.replace(".", "p") + ".csv"
    output_dir = "results/{}/roberta_large/syn_p_h/".format(folder)
