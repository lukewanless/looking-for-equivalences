...
import glob
from lr.models.transformers.XLNetWrapper import XLNetWrapper
...
def run_test(folder,
    ...
    model = XLNetWrapper(hyperparams)
...
if __name__ == '__main__':
    ...
    search_path = "hyperparams/xlnet_base_{}/search_{}.csv".format(folder,
                                                                   search_random_state)
    assert os.path.exists(search_path)

    transformation_name = "wordnet syn tranformation p and h"
    output_raw_result = "raw_results/{}/xlnet_base/syn_p_h/rho_{:.2f}_results".format(folder,
                                                                                      rho)
    output_raw_result = output_raw_result.replace(".", "p") + ".csv"
    output_result = "results/{}/xlnet_base/syn_p_h/rho_{:.2f}_results".format(folder,
                                                                              rho)
    output_result = output_result.replace(".", "p") + ".csv"
    output_dir = "results/{}/xlnet_base/syn_p_h/".format(folder)
