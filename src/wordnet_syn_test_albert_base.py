...
import glob
from lr.models.transformers.AlbertWrapper import AlbertWrapper
...
def run_test(folder,
    ...
    model = AlbertWrapper(hyperparams)
...
if __name__ == '__main__':
    ...
    search_path = "hyperparams/albert_base_{}/search_{}.csv".format(folder,
                                                                    search_random_state)
    assert os.path.exists(search_path)

    transformation_name = "wordnet syn tranformation p and h"
    output_raw_result = "raw_results/{}/albert_base/syn_p_h/rho_{:.2f}_results".format(folder,
                                                                                       rho)
    output_raw_result = output_raw_result.replace(".", "p") + ".csv"
    output_result = "results/{}/albert_base/syn_p_h/rho_{:.2f}_results".format(folder,
                                                                               rho)
    output_result = output_result.replace(".", "p") + ".csv"
    output_dir = "results/{}/albert_base/syn_p_h/".format(folder)
